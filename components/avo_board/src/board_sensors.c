...
/* Accelerometer at 500 Hz without its low-pass: a knock on the case stays a
 * sharp 1-2.5 g spike in our 100 Hz reads (measured). The chip's own tap
 * engine never reported a tap on this board, so detection is in avo_core.
 * Steps and raise-to-wake apply their own software low-pass. */
